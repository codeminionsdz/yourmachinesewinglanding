...
alter table public.products
  add constraint products_colors_array check (jsonb_typeof(colors) = 'array'),
  add constraint products_sizes_array check (jsonb_typeof(sizes) = 'array'),
  add constraint products_media_object check (jsonb_typeof(media) = 'object');

update public.products
set active = false, updated_at = now()
where slug = 'acme-model-320';

insert into public.products (name, slug, brand, manufacturer, model, description, price, currency, active, colors, sizes, media)
values (
  'GURM Convertible Cargo Pants',
  'gurm-convertible-cargo-pants',
  'GURM',
  'GURM',
  'Convertible Cargo Pants',
  'سروال Cargo عملي قابل للتحويل.',
  null,
  'DZD',
  false,
  '[{"name":"Black","label":"أسود","swatch":"#101315"},{"name":"Grey","label":"رمادي","swatch":"#747a78"},{"name":"Olive","label":"زيتي","swatch":"#68705a"}]'::jsonb,
  '["S","M","L","XL","XXL"]'::jsonb,
  '{"hero":"/gurm/hero-motorcycle.jpg","black":"/gurm/product-black.jpg","grey":"/gurm/product-grey.jpg","olive":"/gurm/product-olive.jpg","shorts":"/gurm/convertible-shorts.jpg","detail":"/gurm/detail-cargo-pocket.jpg","fullBody":"/gurm/lifestyle-full-body.jpg","motorcycle":"/gurm/lifestyle-motorcycle.jpg","colors":"/gurm/gallery-multiple-colors.jpg"}'::jsonb
)
...
begin
  ...
  values (v_order_number, v_submission_id, v_customer.id, v_product.id, p_quantity, v_product.price, v_product.price * p_quantity, v_product.currency, trim(p_wilaya), trim(p_commune), trim(p_address), nullif(trim(p_notes), ''), p_utm_source, p_utm_medium, p_utm_campaign, p_utm_content, p_utm_term, p_fbclid)
