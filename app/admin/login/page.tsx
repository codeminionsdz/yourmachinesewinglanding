...
export default function AdminLogin() {
  ...
  return <main className="admin-login" dir="ltr"><form onSubmit={submit}><div className="admin-login-brand">Dani Wear<small>Commerce workspace</small></div><h1>Admin sign in</h1><p>Sign in to manage Dani Wear orders and store operations.</p>{error && <div className="error">{error}</div>}<label>Email<input type="email" required value={email} onChange={e => setEmail(e.target.value)} autoComplete="email" /></label><label>Password<input type="password" required value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password" /></label><button disabled={busy}>{busy ? 'Signing in…' : 'Sign in'}</button></form></main>
}
