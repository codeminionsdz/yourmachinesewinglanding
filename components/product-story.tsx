...
const SectionTitle=({kicker,title,children}:{kicker:string,title:string,children?:React.ReactNode})=><div className="max-w-2xl"><p className="eyebrow">{kicker}</p><h2 className="headline">{title}</h2>{children ? <p className="section-copy">{children}</p> : null}</div>
 ...
 return <main>
  ...
  <section id="top" className="hero-commerce border-b border-[var(--border)]"><motion.img initial={{scale:1.03}} animate={{scale:1}} transition={{duration:1.2}} src={p.hero} alt="ماكينة أوفرلوك Moustex Model 320 كاملة"/><div className="hero-panel"><p className="eyebrow hero-model">ACME model 320</p><h1>سورجي 4 خيوط منزلية بجودة صناعية</h1><p className="hero-description">ماكنة سرفلة منزلية تسورجي و تخيط و تقص القماش الزايد بإحترافية.</p><Button/></div><ScrollCue className="hero-cue"/></section>
